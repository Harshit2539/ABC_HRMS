    ...
    <div class="container-fluid mt-4">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">
                            <!-- <h3 class="card-label">Currency Types</h3> -->
                        </div>
                    ...
                    <div class="card-body table-responsive">
                        <table class="table table-striped custom-table datatable CurrencyList" style="font-size:small !important;">
                            <thead>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addNewModalLabel">Currency</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="saveCurrencyBtn" class="btn btn-primary">Save</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="editCurrencyModal" tabindex="-1" aria-labelledby="editCurrencyModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editCurrencyModalLabel">Currency</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                        <button type="button" id="editCurrencyBtn" class="btn btn-primary">Save</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $('#saveCurrencyBtn').on('click', function (e) {
        e.preventDefault();

        var formData = new FormData($('#addCurrencyForm')[0]);
        $('.text-danger').empty();
        $('.pre-loader').show();

        $.ajax({
            url: "{{ route('currency_types.store') }}",
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#editCurrencyBtn').on('click', function (e) {
        e.preventDefault();
        var id = $('#currencyId').val();
        var formData = new FormData($('#editCurrencyForm')[0]);
        $('.text-danger').empty();
        $('.pre-loader').show();

        $.ajax({
            url: `/currency_types_update/${id}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
