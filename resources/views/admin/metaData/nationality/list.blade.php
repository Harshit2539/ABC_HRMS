    ...
    <div class="container-fluid mt-4">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">
                            <!-- <h3 class="card-label">Nationalities</h3> -->
                        </div>
                    ...
                    <div class="card-body table-responsive">
                        <table class="table table-striped custom-table datatable NationalityList"  style="font-size:small !important;">
                            <thead>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addNewModalLabel">Nationality</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="saveNationalityBtn" class="btn btn-primary">Save</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="editNationalityModal" tabindex="-1" aria-labelledby="editNationalityModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editNationalityModalLabel">Nationality</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form id="editNationalityForm">
                            @csrf
                            <input type="hidden" name="id" id="nationalityId">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                        <button type="button" id="editNationalityBtn" class="btn btn-primary">Save</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $('#saveNationalityBtn').on('click', function (e) {
        e.preventDefault();

        var formData = new FormData($('#addNationalityForm')[0]);
        $('.text-danger').empty();
        $('.pre-loader').show();
    ...
    var table = $('.NationalityList').DataTable({
        responsive: true,
        processing: true,
        serverSide: false,
        searching: true,
    ...
    $(document).on('click', '.btn-edit', function () {
        var id = $(this).data('id');
        $.ajax({
            url: `/nationalities_edit/${id}`,
            type: 'GET',
            success: function (data) {
                $('#nationalityId').val(data.id);
    ...
    $('#editNationalityBtn').on('click', function (e) {
        e.preventDefault();
        var id = $('#nationalityId').val();
        var formData = new FormData($('#editNationalityForm')[0]);
        $('.text-danger').empty();
        $('.pre-loader').show();

        $.ajax({
            url: `/nationalities_update/${id}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.result === 'success') {
                    ...
                    alert('Failed to delete the Nationality. Please try again.');
                }
            },
            error: function () {
