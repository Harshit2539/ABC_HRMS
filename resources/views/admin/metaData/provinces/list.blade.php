                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">
                            <!-- <h3 class="card-label">Provinces</h3> -->
                        </div>
                    ...
                    <div class="card-body table-responsive">
                        <table class="table table-striped custom-table datatable ProvinceList" style="font-size:small !important;">
                            <thead>
                            <tr style="background: linear-gradient(90deg, rgb(255, 81, 81) 0%, rgb(220, 185, 255) 35%, rgb(255, 133, 133) 100%);">
                            <th class="text-center">Sr No</th>
                                    <th class="text-center">Code</th>
                                    <th class="text-center">Name</th>
                                    <th class="text-center">Countary</th>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addNewModalLabel">Province</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <form id="addProvinceForm">
                            ...
                            <div class="form-group">
                                <label for="code"><span class="mandatory_input" style="color:red;">*</span> Code:</label>
                            ...
                            <div class="form-group">
                            <label for="countary_id"><span class="mandatory_input" style="color:red;">*</span> Countary: </label>
                                <select class="form-control" id="countary_id" name="countary_id">
                                    <option value="">Select Countary</option>
                                    @foreach($countries as $countary)
                                   <option value="{{$countary->id}}">{{$countary->name}}</option>
                                   @endforeach
                                </select>
                                <small class="text-danger countary_id_error"></small>
                            </div>
                        </form>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="saveProvinceBtn" class="btn btn-primary">Save</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="editProvinceForm">
                            ...
                            <div class="mb-3">
                                <label for="editCode" class="form-label"><span class="mandatory_input" style="color:red;">*</span> Code</label>
                            ...
                            <div class="form-group">
                            <label for="editCountary"><span class="mandatory_input" style="color:red;">*</span> Countary: </label>
                                <select class="form-control" id="editCountary" name="countary_id">
                                   <option value="">Select Countary</option>
                                    @foreach($countries as $countary)
                                   <option value="{{$countary->id}}">{{$countary->name}}</option>
                                   @endforeach
...
<script>
    ...
    $('#saveProvinceBtn').on('click', function (e) {
        e.preventDefault();

        var formData = new FormData($('#addProvinceForm')[0]);
        $('.text-danger').empty();
        $('.pre-loader').show();
    ...
    $(document).on('click', '.btn-edit', function () {
        var id = $(this).data('id');
        $.ajax({
            url: `/provinces_edit/${id}`,
            type: 'GET',
            success: function (data) {
                $('#provinceId').val(data.id);
                $('#editName').val(data.name);
                $('#editCode').val(data.code);
                $('#editCountary').val(data.countary_id);
    ...
    $('#editProvinceBtn').on('click', function (e) {
        e.preventDefault();
        var id = $('#provinceId').val();
        var formData = new FormData($('#editProvinceForm')[0]);
        $('.text-danger').empty();
        $('.pre-loader').show();

        $.ajax({
            url: `/provinces_update/${id}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.result === 'success') {
                    ...
                    alert('Failed to delete the Province. Please try again.');
                }
            },
            error: function () {
