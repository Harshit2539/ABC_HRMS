    ...
    <div class="container-fluid mt-4">
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body table-responsive">
                        <table class="table table-striped custom-table datatable loanTypeList" style="font-size:0.9em !important;">
                            <thead>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addNewModalLabel">Loan Type</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" id="saveLoanTypeBtn" class="btn btn-primary">Save</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="editLoanTypeModal" tabindex="-1" aria-labelledby="editLoanTypeModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editLoanTypeModalLabel">Loan Type</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                        <button type="button" id="editLoanTypeBtn" class="btn btn-primary">Save</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $('#saveLoanTypeBtn').on('click', function (e) {
        e.preventDefault();

        var formData = new FormData($('#addLoanTypeForm')[0]);
        $('.text-danger').empty();
        $('.pre-loader').show();

        $.ajax({
            url: "{{ route('loan_types.store') }}",
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    var table = $('.loanTypeList').DataTable({
        responsive: true,
        processing: true,
        serverSide: false,
        searching: true,
        ajax: {
            url: "{{ route('loan_types.list') }}",
        },
        columns: [
    ...
    $('#editLoanTypeBtn').on('click', function (e) {
        e.preventDefault();
        var id = $('#loanTypeId').val();
        var formData = new FormData($('#editLoanTypeForm')[0]);
        $('.text-danger').empty();
        $('.pre-loader').show();

        $.ajax({
            url: `/loan_types_update/${id}`,
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.result === 'success') {
                    ...
                    alert('Failed to delete the Loan Type. Please try again.');
                }
            },
            error: function () {
